...
import { verify_array_type, validate_price, validate_bid } from '$lib/server/utils';
 ...
 */
function auth_user_request({name, pass, passNew})
{
    ...
    const creds = querys.getUserCredsByName.get(name);
 ...
 */
function user_command_trans({id, cId, balance}, {pId, amount, notes})
{
    ...
    const p = validate_price(pId, cId, ['Purchase']);
    ...
    querys.userTransact.run(total, now, id);
    querys.addPurchase.run(id, cId, p.desc, notes ?? '', amount, now);
    
    querys.addLog.run(id, p.desc, p.type, notes ?? '', balance+total, total, now);
...
/**
 * @param {{id: number, cId: number, balance: number}} user 
 * @param {{bId: number, amount: number, notes?: string}} args 
 * @returns 
 */
function user_command_bid({id, cId, balance}, {bId, amount, notes})
{
    if(typeof bId != 'number') error(422, "Missing/Malformed value: args.bId");
    if(typeof amount != 'number') error(422, "Missing/Malformed value: args.amount");
    
    const b = querys.getBidById.get(bId);
    ...
    querys.addPlacedBid.run(id, cId, bId, amount, now);
...
export async function POST({cookies, request, params})
{
    ...
    const user = querys.getUserById.get(userId);
